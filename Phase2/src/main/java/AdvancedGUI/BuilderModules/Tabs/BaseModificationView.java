...
public class BaseModificationView extends BorderPane{
    ...
    public BaseModificationView(GolfCourse course, double[] preViewSize) {
        ...
        ChangeListener<String> autoUpdateListener = (observable, oldValue, newValue) -> {
            String xText = widthXString.getText();
            String yText = widthYString.getText();
            String formula = terrainFormula.getText();

            // Verify both fields are filled in before parsing
            if (xText == null || 
                xText.trim().isEmpty() || 
                yText == null || 
                yText.trim().isEmpty() ||
        ...
        widthXString.textProperty().addListener(autoUpdateListener);
        widthYString.textProperty().addListener(autoUpdateListener);
        terrainFormula.textProperty().addListener(autoUpdateListener);
