...
public class GolfCourse {
    ...
    // private double miuS; // static friction coeficient, theoretically its more
    // like a constant. I just put it here to have both mius in one place
    ...
    private String terrainFormula = "0.25*sin((x-y)/10.0)+1"; // chage this to get startup course
    ...
    public double[] getDerivative(double x, double y) {
        double slopeX = (calculateHeight(terrainFormula, x + epsilon, y) - calculateHeight(terrainFormula, x, y))
                / epsilon;
        double slopeY = (calculateHeight(terrainFormula, x, y + epsilon) - calculateHeight(terrainFormula, x, y))
                / epsilon;
    ...
    // we pass different things around in classes so we nee dthe same method twice
    // but slightly different input
