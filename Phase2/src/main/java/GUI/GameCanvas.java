...
public class GameCanvas {
    ...
    //we don't want target or ball to sit right on the edge of the canvas, so we add a margin
    ...
    //we use this mehod to make sure that scaling and worl coordinates work for the new course and we can see the target and ball
    private void fitToCurrentCourse() {
        ...
        //need to make sure the largest world dimension fits into the canvas, so we calculate the required metersPerPixel for both dimensions 
        double mppX = worldW / CANVAS_W;
        double mppY = worldH / CANVAS_H;
        metersPerPixel = Math.max(mppX, mppY); //use the larger metersPerPixel to ensure everything fits within the canvas
    ...
    public void drawObjects(double[] ballState, double[][] trajectory, double arrowVx, double arrowVy) {
        ...
        gc.setLineWidth(1.0);
        gc.strokeOval(bpx - BALL_RADIUS_PIXELS, bpy - BALL_RADIUS_PIXELS, BALL_RADIUS_PIXELS * 2, BALL_RADIUS_PIXELS * 2);
    ...
    private void drawTrail(GraphicsContext gc, double[][] trajectory) {
        ...
        for (int i = 1; i < total; i += 3) {
            ...
            //the fraction of the way through the trajectory (0 to 1) -> determines width of pyramid at current point and opacity of the fill color
    ...
    //helps the user have a sense of the scale of the terrain and where the target is in relation to the ball
